...
This page describes the basics on how to use `@mathix420/graphql` as part of a https://nextjs.org[Next.js] application.  
...
At this point it's time to add the dependencies needed to run `@mathix420/graphql` inside Next.js.
...
== Start the server
That is all that needs to be done for `@mathix420/graphql` to work in a Next.js application. The dev server starts wtih `yarn dev` and Apollo Studio should be loaded on http://localhost:3000 and the front-end part of the Next.js application should be able to query the API.
