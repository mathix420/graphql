...
In `@mathix420/graphql`, the ethos has been to simplify the top-level arguments:
...
Is now changed to the following using `@mathix420/graphql`:
...
[cols="1,1"]
|===
|`neo4j-graphql-js` |`@mathix420/graphql`
...
Sorting has changed somewhat in `@mathix420/graphql`. For the example being used in this page, the `_MovieOrdering` type in `neo4j-graphql-js` was an enum which looked like the following:
...
In `@mathix420/graphql`, the sorting type `MovieSort` has become an input type with each field as an enum, like follows:
...
Using `@mathix420/graphql`, this will now be:
