...
describe("Cypher Auth Roles", () => {
    ...
    test("Read Node & Cypher Field", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            WITH this
            CALL apoc.util.validate(NOT(ANY(r IN [\\"super-admin\\"] WHERE ANY(rr IN $auth.roles WHERE r = rr))), \\"@mathix420/graphql/FORBIDDEN\\", [0])
            RETURN this { history: [this_history IN apoc.cypher.runFirstColumn(\\"MATCH (this)-[:HAS_HISTORY]->(h:History) RETURN h\\", {this: this, auth: $auth}, true) WHERE apoc.util.validatePredicate(NOT(ANY(r IN [\\"super-admin\\"] WHERE ANY(rr IN $auth.roles WHERE r = rr))), \\"@mathix420/graphql/FORBIDDEN\\", [0]) | this_history { .url }] } as this"
        `);
